...
from left_join.hash_table import HashTable
...
def test_left_join():
    ...
    assert left_join(one, two) == [['diligent', 'employed', 'idle'], ['wrath', 'anger', 'delight'], [
        'guide', 'usher', 'follow'], ['fond', 'enamored', 'averse'], ['outfit', 'garb', None]]
...
def test_left_join_if_no_any_matching():
    ...
    assert left_join(one, two) == [['poutfit', 'garb', None], ['hangguide', 'usher', None], [
        'rath', 'anger', None], ['pond', 'enamored', None], ['adiligent', 'employed', None]]
